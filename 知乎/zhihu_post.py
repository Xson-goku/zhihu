...
from zhihu_base import BaseZhihu, JsonAsSoupMixin


class Zhihu_post(JsonAsSoupMixin, BaseZhihu):
    ...
    def column(self):
        ...
        :rtype: Column
        """
        from zhihu_column import Zhihu_column

        if 'column' in self.soup:
            url = Zhihu_column_Url + '/' + self.soup['column']['slug']
            name = self.soup['column']['name']
            return Zhihu_column(url, name, session=self._session)
        else:
            return None
    ...
    def author(self):
        ...
        return Zhihu_author(url, name, motto, photo_url=photo_url,
